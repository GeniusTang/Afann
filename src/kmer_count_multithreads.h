...
void count_one_read(int id, int K, std::string one_read, std::vector<std::atomic<int>> &count_array, bool Reverse) {
    ...
    for (int i=0;i<length;i++){ 
		...
		if (Reverse) rev = (rev >> 2) + (3-nuc2num[nuc]) * pow(4, K-1);
                j += 1;
	    }
            else{
                num = (num&mask)<<2;
		...
		if (Reverse) {
		    ...
		    count_array[rev] ++;
		    */
	            rev = (rev >> 2) + (3-nuc2num[nuc]) * pow(4, K-1);
		    count_array[rev] ++;
		}
	   }
        }
...
void count_one_read_M_K(int id, int M, int K, std::string one_read, std::vector<std::atomic<int>> &count_array, bool Reverse) {
    ...
    for (;i<length;i++){
	    ...
	    if (j < (M-1)){
		num_K = num_K * 4 + nuc2num[nuc];
		if (Reverse) rev_K = (rev_K >> 2) + (3-nuc2num[nuc]) * pow(4, K-1);
		j += 1;
	    }
	    else {
                if (j < (K-1)){
		    ...
		    if (Reverse) rev_K = (rev_K >> 2) + (3-nuc2num[nuc]) * pow(4, K-1);
                    ...
                    if (Reverse) {
			rev_K = (rev_K >> 2) + (3-nuc2num[nuc]) * pow(4, K-1);
...
std::vector<std::atomic<int>> count_M_K(std::string filename, int M, int K, int Num_Threads, bool Reverse) {
    ...
    while (true) {
        ...
        if (one_read.length() >= READ_LENGTH){
            ...
            one_read = "B" + one_read.substr(one_read.length()-K+1);
        }
        if (fs.eof()) break;
        fs.clear();
    }
